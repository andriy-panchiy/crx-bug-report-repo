...
function removeSrcFromHtmlPaths(): PluginOption {
  ...
  return {
    ...
    closeBundle() {
      ...
      for (const key of ['action.default_popup', 'options_ui.page', 'options_page', 'devtools_page', 'chrome_url_overrides', 'web_accessible_resources']) {
      ...
      if (glob.sync(`${config.build.outDir}/src/**/*.html`).length > 0) {
        ...
        throw new Error('Something went wrong. Files found in src folder, please open an issue in GitHub.');
      } else {
